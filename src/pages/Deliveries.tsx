...
import { ThirdPartyProviderManager } from '@/components/deliveries/ThirdPartyProviderManager';
...
export default function Deliveries() {
    ...
    useEffect(() => {
        ...
        if (orderId && deliveries.length > 0 && selectedDeliveryId !== orderId) {
    ...
    const defaultCenter = useMemo<[number, number]>(() => [-1.286389, 36.817223], []);
    ...
    const [activeTab, setActiveTab] = useState<'tracking' | 'providers'>('tracking');
    ...
    return (
        <div className="h-[calc(100vh-4rem)] flex flex-col p-6 space-y-6">
                    ...
                    <div className="bg-muted p-1 rounded-xl flex items-center">
                        <button
                            ...
                            className={cn(
                                ...
                                activeTab === 'tracking' ? "bg-card shadow-sm text-primary" : "text-muted-foreground hover:text-foreground"
                            )}
                        >
                        ...
                        <button
                            ...
                            className={cn(
                                "flex items-center gap-2 px-4 py-2 rounded-lg transition-all text-sm font-bold",
                                activeTab === 'providers' ? "bg-card shadow-sm text-primary" : "text-muted-foreground hover:text-foreground"
                            )}
                        >
            ...
            {activeTab === 'providers' ? (
                <div className="flex-1 overflow-y-auto min-h-0 bg-card/30 rounded-3xl border border-border/50 p-6">
