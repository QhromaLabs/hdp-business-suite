...
import { Loader2, PlayCircle, Scale, BookOpenCheck, ChevronDown, ChevronRight, ChevronLeft, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
export default function GeneralLedgerPanel({ dateRange }: GeneralLedgerPanelProps) {
  ...
  const { data: jeData, isLoading: jeLoading } = useJournalEntries(currentPage, pageSize);
  const journalEntries = jeData?.entries || [];
  const totalCount = jeData?.totalCount || 0;
                    ...
                    {(() => {
                      ...
                      return pages.map((p, idx) =>
                        p === 'ellipsis-start' || p === 'ellipsis-end' ? (
                          <span key={idx} className="min-w-[36px] h-9 flex items-center justify-center text-muted-foreground text-sm font-bold select-none">…</span>
                        ) : (
                          <button
                            key={p}
                            onClick={() => {
