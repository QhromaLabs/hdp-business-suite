...
BEGIN
    ...
    -- Cost of goods sold, from the FIFO landed cost captured per line item at the moment
    -- of sale (sales_order_items.landed_cost_at_sale), not product_variants.cost_price --
    -- cost_price is a manually-edited reference value that drifts from what inventory_batches
    -- actually consumed, while landed_cost_at_sale is exactly what was relieved from
    -- Finished Goods Inventory (1030) by the FIFO deduction trigger.
