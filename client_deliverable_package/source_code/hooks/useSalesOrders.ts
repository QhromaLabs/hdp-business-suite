...
export function useSoftDeleteSalesOrder() {
  ...
  return useMutation({
    mutationFn: async ({ orderId, items }: { orderId: string; items?: any[] }) => {
      ...
      // 2. Restock items into inventory
      ...
      // 3. Mark sales order status as 'deleted'
      const { error } = await supabase
        .from('sales_orders')
    ...
    onSuccess: (_data, variables) => {
      ...
      queryClient.setQueriesData({ queryKey: ['sales_orders'] }, (existing: any) => {
        ...
        if (Array.isArray(existing)) {
          return existing.filter((order: any) => order.id !== orderId);
        }
        if (existing.orders && Array.isArray(existing.orders)) {
          return {
            ...existing,
            orders: existing.orders.filter((order: any) => order.id !== orderId),
      ...
      queryClient.invalidateQueries({ queryKey: ['dashboard_stats'] });
      queryClient.invalidateQueries({ queryKey: ['inventory'] });
      toast.success('Order moved to Deleted tab & stock restocked');
    },
    onError: (error: any) => {
      toast.error('Failed to move order to deleted: ' + error.message);
    },
  });
}
...
export function useRestoreSalesOrder() {
  ...
  return useMutation({
    mutationFn: async ({ orderId }: { orderId: string }) => {
      ...
      // 2. Deduct items back from inventory when restored
      for (const item of items || []) {
        ...
        if (invData) {
          await supabase
            .from('inventory')
            .update({ quantity: Math.max(0, (invData.quantity || 0) - (item.quantity || 0)) })
            .eq('variant_id', item.variant_id);
        }
      }

      // 3. Restore status to 'pending'
      const { error } = await supabase
        .from('sales_orders')
...
export function useDeleteSalesOrder() {
  ...
  return useMutation({
    mutationFn: async ({ orderId, items }: { orderId: string; items?: any[] }) => {
      // 1. Fetch items if not provided or empty
      let orderItems = items;
      if (!orderItems || orderItems.length === 0) {
        const { data: fetchedItems } = await supabase
          .from('sales_order_items')
      ...
      // 2. Restock items safely using maybeSingle to avoid exceptions
      for (const item of orderItems) {
        if (!item?.variant_id) continue;
        const { data: invData } = await supabase
          .from('inventory')
      ...
      // 3. Delete related sales commissions
      console.log('Deleting sales commissions for order:', orderId);
      ...
      if (commError) {
        console.error('Error deleting sales commissions:', commError);
      }

      // 4. Delete related sales feedback if present
      console.log('Deleting sales feedback for order:', orderId);
      ...
      if (feedbackError) {
        console.error('Error deleting sales feedback:', feedbackError);
      }

      // 5. Delete related payments first (Foreign Key constraint blocker)
